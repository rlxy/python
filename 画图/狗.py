from turtle import *
pensize(5)  #设置画笔的宽度
speed(5)    #设置画笔移动速度，画笔绘制的速度范围[0,10]整数，数字越大越快
##color('#F4A460')#橘黄
##color('#FFE4E1')#肉粉

#每一个turtle都是一支笔，不是落笔就是抬笔

##【背景圆】
color('#B088FF')#浅紫
pu()            #抬笔，(pen up)
goto(0,-200)    #将画笔移动到坐标为x,y的位置
pd()            #落笔，(pen down)
begin_fill()    #准备开始填充图形；
circle(200)     #背景圆的大小
end_fill()      #结束填充图形
...
def Arc(initial_degree,range_num,step,rotate_degree):
    seth(initial_degree)        #画笔指向变为坐标系绝对角度initial_degree
    for n in range(range_num):   
        fd(step)        #前进step个像素
        rt(rotate_degree)#向右转，接受的实参是角度
 
##定义填充矩形函数
def Rect(x,y,height,width):
    pu()        #抬笔
    goto(x,y)   #把笔移动到坐标x，y的位置
    pd()        #落笔
    begin_fill()#开始填充图形
    goto(x+width,y)

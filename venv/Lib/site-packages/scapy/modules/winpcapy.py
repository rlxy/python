...
#   Open a live capture from the scapy_.
...
try:  # NPCAP ONLY function
    ...
    #   create a packet capture handle to look at packets on the scapy_.
...
#   Construct a list of scapy_ devices that can be opened with pcap_open_live().  # noqa: E501
...
if WINDOWS:
    ...
    #   Send a queue of raw packets to the scapy_.
    ...
    #   Create a list of scapy_ devices that can be opened with pcap_open().
    ...
    #   Block until a scapy_ connection is accepted (active mode only).
